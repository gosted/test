...
(function($, w, d){
	...
	$(function() {
		function AssetCheck(options){
			...
			this.list = [];
			this.STr = null;
			this.equId = parent.window.layerViewData.equId;
			this.assetStatus = parent.window.layerViewData.assetStatus;
		...
		AssetCheck.prototype = {
							...
							laypage({
								...
								jump: function(obj, first){ //触发分页后的回调
									if(!first){ //点击跳页触发函数自身，并传递当前页：obj.curr
										...
										sendData.equipmentId = that.equId;
										sendData.assetStatus = that.assetStatus;
										$.myAjax({
											type: "POST",
											url: window.ajaxUrl + "/project/assetRegistration/findPageAssetRegistration",
											data: sendData,
			...
			setdata:function(data)
			{
				...
				$.each(this.list, function (i, v)
				{
					...
					this.STr.append($('<td></td>').text(v.quantity==null?"":v.quantity));
