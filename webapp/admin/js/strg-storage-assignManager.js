...
(function($, w, d){
	...
	$(function() {
		var storageId = window.parent.layerViewData.strgRoomId,
		//var storageId = "1234567",
		    selected = false,
			tbody = $(".tbody");
		...
		function setTable (data)
		{
			...
			$.each(list, function (i, v)
			{
				...
				STr.append($('<td></td>').text(v.unitName  || ""));
		...
		function renderingPage (sendData)
		{
			$.myAjax({
				type: "POST",
				url: window.ajaxUrl + "operation/storageKeeper/find",
				data: sendData,
		...
		$(".add-btn").on("click", function()
		{
			...
			$.myAjax({
				type: "POST",
				url: window.ajaxUrl + "operation/storageKeeper/create",
				data: sendData,
				success: function (data) {
					if (data && data.success === 0) {
		...
		tbody.on("click", ".btns .delete", function ()
		{
			...
			layer.confirm('确定要删除吗？', {
				...
				function ()
				{
					...
					$.myAjax({
						type: "POST",
						url: window.ajaxUrl + "operation/storageKeeper/delete",
						data: {
							userId: userId,
							storageId: storageId
						},
