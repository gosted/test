...
(function($, w, d) {
    ...
    $(function(){
        var pageSize = 1000,
            ...
            complainantId = parent.window.layerViewData.complaintId,
        ...
        $("#table-box").Validform({
            ...
            beforeSubmit:function(curform){
                ...
                else
                {
                    ...
                    sendData.recordId = complainantId;
                    sendData.attachmentId = strId;
                    $.myAjax({
                        type: "POST",
                        url: window.ajaxUrl + "operation/complaintTreatment/create",
                        data: sendData,
                        dataType: "json",
                        success:function(data)
                        {
        ...
        $.myAjax({
            type: "POST",
            url: window.ajaxUrl +"operation/complaintTreatment/find",
            data: {recordId:complainantId},
            dataType: "json",
            success: function(data)
            {
                    ...
                    $.each(dataList,function (i,v) {
                        ...
                        if (v.attachmentId != undefined || v.attachmentId != null || v.attachmentId != "") {
                            var str = "<tr><td colspan='5' class='table-key'>处理人：</td><td colspan='15'>" + (dataList[i].treatPeople || "") + "</td><td colspan='5' class='table-key'>联系方式：</td><td colspan='15'>" + (dataList[i].treatPhone || "") + "</td></tr>" +
                                "<tr><td colspan='5' class='table-key'>处理时间：</td><td colspan='15'>" + window.formatDates(dataList[i].treatTime) + "</td><td colspan='5' class='table-key'>状态：</td><td colspan='15'>" + (dataList[i].result || "") + "</td></tr>" +
