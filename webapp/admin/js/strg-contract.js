	...
	$(function() {
		...
		function setTable (data)
		{
			...
			$.each(list, function (i, v)
			{
				...
				STr = $('<tr class="text-c" contractId="'+ contractId+'"></tr>');//一行
				STr.append('<td><input type="checkbox" value="" name=""></td>');
				//STr.append($('<td class="text-l td-view"></td>').append(view.text(v.contractName || "")));
				STr.append($('<td class="text-l td-view"></td>').text(v.contractName || ""));
				STr.append($('<td></td>').text(v.contractCode || ""));
				STr.append($('<td></td>').text(window.formatDates(v.performTime) || ""));
				STr.append($('<td></td>').text(v.contractA || ""));
				STr.append($('<td></td>').text(v.contractB || ""));
		...
		function renderingPage (sendData)
		{
			$.myAjax({
				type: "POST",
				url: window.ajaxUrl + "project/contract/findPageInFixed",
				data: sendData,
		...
		function findList()
		{
			...
			contractCode = search.find('[con_name="contractCode"]').val();
			contractName = search.find('[con_name="contractName"]').val();
		...
		tbody.on("click", ".btns .sure", function ()
		{
			var _this = $(this),
				contractId = $(this).parents("tr").attr("contractId"),
				strgRoomName = $(this).parents("tr").find(".td-view").eq(0).text(),
