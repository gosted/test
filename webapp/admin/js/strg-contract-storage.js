	...
	$(function() {
				...
				success: function(data)
				{
					...
					if (data && data.success === 0)
					{
						var arr = [
							...
							contractAmont = data.data.contractAmont,
							amontNoTax = data.data.amontNoTax;
						for (var i = 0, l =arr.length; i < l; i++)
						{
							$('[con_name="'+ arr[i] +'"]').text(data.data[arr[i]] || "");
						}
						$('[con_name="contractAmont"]').text(comdify(contractAmont.toString()) || "");
						$('[con_name="amontNoTax"]').text(comdify(amontNoTax.toString()) || "");
						$('[con_name="performTime"]').text(window.formatDates(data.data.performTime) || "");
						$('[con_name="endTime"]').text(window.formatDates(data.data.endTime) || "");
						...
						$.myAjax({
							...
							success: function(res)
							{
								...
								{
									setContractDetailList({contractId: sourceId});//渲染合同清单基本信息
									for (var i = 0, l = res.data.length; i < l; i++)
									{
										if (data.data.currency === res.data[i].dictCodeValue)
										{
											$('[con_name="currency"]').text(res.data[i].dictCodeName || "");
											break;
										}
									}
								}
		...
		function setTable (data)
		{
			...
			$.each(list, function (i, v)
			{
				...
				if (v.ifStore === 0 && v.unStoreCount > 0)
				{
				...
				STr.append($('<td></td>').text(v.detailPrice || ""));
				...
				{
					noInSpan.text(v.unStoreCount || "0");
				}
				...
				tmpBtn += '<a style="text-decoration:none" class="ml-5 storage" href="javascript:;" title="入库" _href="strg-contract-storage-more.html">'+
		...
		function setContractDetailList(sendData)
		{
			$.myAjax({
				type: "POST",
				url: window.ajaxUrl + "project/contract/findDetailContainsNum",
				//url: window.ajaxUrl + "data/storage/contractList.json",
				data: sendData,
				dataType: "json",
				success: function(data)
				{
		...
		//主顺序
		setContractInfo ({id: sourceId});//渲染合同基本信息
		...
		window.setEquDetailList = function (data)
		{
			...
			function setEquDelListOne (obj)
			{
				...
				for (var j = 0, len = temp.length; j < len; j++)
				{
					oneList.find('[con_name="' + temp[j] + '"]').text(obj[temp[j]] || "").attr({"title": obj[temp[j]] || ""});
				}
				oneList.find('[con_name="quantity"]').text(obj.storeCount).attr({"title": obj.storeCount});
				oneList.find('[con_name="equipmentId"]').text(obj.equipmentType || "").attr({
					"title": obj.equipmentType || "",
					"thisId": obj.equipmentId
				});
				oneList.find('[con_name="shelfId"]').text(obj.shelfPosi || "").attr({
					"title": obj.shelfPosi || "",
					"thisId": obj.shelfId
				});
				...
				if (obj.attachmentId)
				{
					oneList.find(".equ-logo").css("background", 'url('+ window.ajaxUrl +'project/attachment/downloadImage?id=' + obj.attachmentId + ') no-repeat').attr({"attachmentId": obj.attachmentId});
				}
		...
		$('.equ-det-list').on("click", ".edit-disable .deal", function()
		{
			...
			if (OLi.find(".edit-able").size() === 0)
			{
				...
				editAble.find('.sure').on("click", function ()
				{
					...
					{
						temp = editAble.find('[con_name="'+ reqArr[i] +'"]').val().replace(/\s*/g,"");
						if (!reqReg.test(temp))
						{
					...
					temp = editAble.find('[con_name="shelfId"]').val().replace(/\s*/g,"");
					if (!regArea.test(temp))
					{
						layer.confirm('请正确选择货架', {
							btn: ['确定'],
							shade: 0.1
						});
						return false;
							...
							$.myAjax({
								...
								success: function(msg)
								{
									if (msg && msg.success === 0)
									{
										...
										$.myAjax({
											type: "POST",
											url: window.ajaxUrl + "operation/storedetail/createSD",
											data: {records: "[" + sndDt + "]"},
											dataType: "json",
											success: function(msg)
											{
