...
(function($, w, d){
	...
	$(function() {
					...
					success: function(msg)
					{
						if (msg && msg.success === 0)
						{
							...
							$.each(list, function (i, v)
							{
								...
								tmpBtn += '<a style="text-decoration:none" class="mr-5 point" href="javascript:;" title="指定项目经理">'+
									'<i class="Hui-iconfont">&#xe60d;</i></a>';
		...
		function renderingPage (sendData)
		{
			$.myAjax({
				type: "POST",
				url: window.ajaxUrl + "project/project/findPage",
				data: sendData,
		...
		tbody.on("click", ".btns .point", function ()
		{
			...
			data.parentProject = parentProject;
			window.layerViewData = data;
			window.layerShow(title,"pro-point-manager.html",null,null,true);
		});
		...
		tbody.on("click", ".btns .delete", function ()
		{
			...
			layer.confirm('确定要删除吗？', {
				...
				function ()
				{
					...
					projectId = _this.parents("tr").attr("projectId");

					$.myAjax({
						type: "POST",
						url: window.ajaxUrl + "project/project/deleteByIds",
						data: {ids: projectId},
						dataType: "json",
						success: function(msg)
						{
