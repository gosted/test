...
(function($, w, d){
	...
	$(function() {
		...
		$(".store").on("click", function ()
		{
			...
			{
				var ind = layer.confirm('该操作会覆盖现有入库确认单，是否继续？', {
						btn: ['确定','取消'],
						shade: 0.1
					},
					function ()
					{
						...
						childLayerInd = parent.layer.open({
							...
							shade:0.4,
							title: "入库",
							content: "strg-unfounded-storage-more.html",
							end: function ()
							{
		...
		$('.equ-det-list').on("blur", "textarea", function ()
		{
		...
		function reorder ($ele)
		{
			...
			asCode = equDelList.find('.edit-able [con_name="assetCode"]').eq(0);
			if (!asCode.val())
			{
				asCode.focus();
			}
