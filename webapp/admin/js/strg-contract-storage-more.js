...
(function($, w, d){
	...
	$(function() {
		var sourceId = parent.window.layerViewData.sourceId,
			listId = parent.window.layerViewData.listId,
		...
		//请求单条合同清单信息
		function setConDetListOne(sendData)
		{
			$.myAjax({
				type: "POST",
				...
				success: function(data)
				{
					...
					{
						var equName = $(".equ-name"),
							equNameSpan = equName.find("span"),
							equCodeSpan = $(".equ-code span"),
							noIn = $(".no-in"),
							allIn = $(".all-in"),
							equVal = $(".equ-val span").eq(0),
							listData = data.data[0];

						/*if (listData.attachmentId)
						{
							equName.css("background", 'url('+ window.ajaxUrl +'project/attachment/downloadImage?id=' + listData.attachmentId + ') no-repeat');
						}*/
						equNameSpan.text(listData.detailName || "").attr("title",listData.detailName || "");
						equCodeSpan.text(listData.detailModel || "").attr("title",listData.detailModel || "");
						noIn.text(listData.unStoreCount || "0").attr("title",listData.unStoreCount || "");
						allIn.text(listData.detailCount || "0").attr("title",listData.detailCount || "");
						equVal.text(listData.detailPrice || "").attr("title",(listData.detailPrice || "") + "元");
						if (listData.unStoreCount == 0)
						{
							noIn.removeClass("c-red");
						}
						allAbleStoreNum = listData.unStoreCount;
						$('[con_name="storeCount"]').val(listData.unStoreCount);
						$('[con_name="manufacturer"]').val(listData.detailCompany);
						$('[con_name="specifications"]').val(listData.detailModel);
						$('[con_name="storeCount"]').focus().blur();
						price = listData.detailPrice || "";
						unit = listData.detailUnit || "";

						assetName = listData.detailName;
					}
				}
			});
		}
		setConDetListOne({id: listId});
		...
		$(".form-ctn").Validform({
			...
			datatype: {
				...
				{
					var reg = /^\d+(\.?\d+)?$/;
					if (gets.search(reg) > -1)
					{
						if (( Number(allAbleStoreNum) >= Number(gets)) && ( Number(gets) > 0))
						{
							return true;
						}
					}
			...
			beforeSubmit:function(curform){
				...
				equListOndData.assetName = assetName;
				equListOndData.storeStyle = storeStyle;
				equListOndData.price = price;
				equListOndData.unit = unit;
