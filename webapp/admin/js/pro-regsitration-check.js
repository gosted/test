...
(function($, w, d){
    ...
    $(function() {
        ...
        window.getTd(tables);
        $(".providerAdd").find(".partnerName").text(projectName);
        $(".providerAdd").find(".site").text(site);
        $.myAjax({
            type: "POST",
            ...
            success: function(data)
            {
                if (data && data.success === 0) {
                    ...
                    $(".assetCategory").text(_data.equipmentName);
                    $(".assetName").text(_data.assetName);
                    var attrackData = JSON.parse(_data.assetTrackInfos);
                    $(".assetCode1").text(_data.assetCode);
                    $(".companyCode").text(_data.companyCode);
                    $(".assetStatus").text(_data.assetStatusName);
                    $(".remark").text(_data.remark);
                    specifications = _data.specifications;
                    id = _data.id;
                    if(attrackData){
                        $(".assetPosition").text(attrackData.assetPosition);
                        $(".installPeople").text(attrackData.installPeople);
                        $(".installTime").text(attrackData.installTime||"");
                    }else{}
        ...
        $(".table-box").Validform({
            ...
            beforeSubmit:function(curform){
                ...
                sendData.id = id;
                var data = {assetPosition:$(".assetPosition").val(),installPeople:$(".installPeople").val(),installTime:$(".installTime").val()};
